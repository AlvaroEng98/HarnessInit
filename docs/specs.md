...
> La idea es seguir un flujo al estido de Kiro-style: requirements → design → tasks → code.
> El código no se escribe hasta que el spec está aprobado por un usuario.
...
| Estado         | Significado                                                    |
|----------------|----------------------------------------------------------------|
| `pending`      | Sin spec. El `sdd_agent_author` es el primero en actuar.        |
| `spec_ready`   | Spec drafted. Esperando aprobación humana. NO se toca código.  |
| `in_progress`  | Spec aprobado. `agent_developer` trabajando.                   |
| `done`         | Código verde, `reviewer_agent` aprobó, sesión cerrada.         |
| `blocked`      | Atascado. Razón en `progress/current.md`.                      |

## La puerta de aprobación del usuario

El flujo automático se detiene **una vez**: cuando el `sdd_agent_author` termina
sus tres archivos, marca la feature como `spec_ready` y para. El usuario
lee `specs/<feature>/` y dice "aprobado" (o pide cambios).

Solo entonces el `orquestador` transiciona `spec_ready → in_progress` y lanza
el `agent_developer`.

```
pending → [sdd_agent_author] → spec_ready → ⏸ usuario → in_progress → [agent_developer → reviewer_agent] → done
```
...
- El `reviewer_agent` comprueba esta correspondencia explícitamente y rechaza
  si falta.

El `agent_developer` documenta el mapa en `progress/impl_<name>.md`:
